...
export function draw2D() {
 ...
 // Hedef açı değerleri (30 Derece)
    ...
    // Eğer viewBlendFactor henüz tanımlı değilse (ilk açılışta) butona göre 0 veya 1 al
    let t = (typeof state.viewBlendFactor === 'number') 
            ? state.viewBlendFactor 
            : (state.is3DPerspectiveActive ? 1 : 0);

    // t değerini 0-1 arasına sabitle (Hata önleyici)
    t = Math.max(0, Math.min(1, t));

    // Matris Katsayılarını Hesapla (Lineer İnterpolasyon - Lerp)
    // Formül: 2D (Identity) ile 3D (Isometric) matrisleri arasında geçiş
    
    // a: X ölçeği (2D'de 1 -> 3D'de isoCos)
    const a = 1 + (isoCos - 1) * t;
    
    // b: Y eğimi (2D'de 0 -> 3D'de -isoSin)
    const b = 0 + (-isoSin - 0) * t;
    
    // c: X eğimi (2D'de 0 -> 3D'de isoCos)
    const c = 0 + (isoCos - 0) * t;
    
    // d: Y ölçeği (2D'de 1 -> 3D'de isoSin)
    // Not: 2D'de Y ölçeği 1'dir, İzometrikte sin(30)=0.5'tir.
