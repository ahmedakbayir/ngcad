...
import { state, dom, WALL_HEIGHT, WALL_THICKNESS, DOOR_HEIGHT } from "./main.js";
...
const doorMaterial = new THREE.MeshStandardMaterial({ color: '#333333', roughness: 0.8 });
...
function createDoorMesh(door) {
    ...
    const doorCenterPos = {
        x: wall.p1.x + dx * door.pos,
        y: wall.p1.y + dy * door.pos,
    };
    ...
    walls.forEach(w => {
        ...
        wallDoors.forEach(door => {
            ...
            if (doorStart > lastPos) {
                wallSegments.push({ start: lastPos, end: doorStart });
            }
            lastPos = door.pos + door.width / 2;
        });

        if (lastPos < wallLen) {
            wallSegments.push({ start: lastPos, end: wallLen });
        }
